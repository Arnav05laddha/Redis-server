...
struct ZSetData {
    ...
    std::vector<std::string> zrange(long long start, long long stop) const {
        ...
        long long idx = 0;
        for (auto& m : ordered) {
            if (idx >= start && idx <= stop) result.push_back(m.member);
            ++idx;
        }
        return result;
    }
...
// ─── Store entry ──────────────────────────────────────────────────────────────
struct StoreEntry {
    enum class Type { String, List, Stream, ZSet } type = Type::String;
    std::string  str_val;
    ListData     list_val;
...
public:
    ...
    std::optional<std::string> get(const std::string& key) {
        ...
        if (is_expired(it->second)) { data_.erase(it); return std::nullopt; }
        if (it->second.type != StoreEntry::Type::String) return std::nullopt;
    ...
    bool del(const std::string& key) {
        ...
        if (erased) watch_reg.notify_write(key);
        return erased;
    }
    ...
    long long pttl(const std::string& key) {
        ...
        if (is_expired(it->second)) { data_.erase(it); return -2; }
    ...
    std::string type_of(const std::string& key) {
        ...
        if (is_expired(it->second)) { data_.erase(it); return "none"; }
        switch (it->second.type) {
            case StoreEntry::Type::String: return "string";
            case StoreEntry::Type::List:   return "list";
            case StoreEntry::Type::Stream: return "stream";
            case StoreEntry::Type::ZSet:   return "zset";
        }
        return "none";
    }
    ...
    // ── List ops ──────────────────────────────────────────────────────────────
    // Returns new list length. dir=1 → rpush, dir=-1 → lpush
    long long list_push(const std::string& key,
        ...
        long long len;
        {
            ...
            for (auto& v : vals) {
                if (dir >= 0) e.list_val.push_back(v);
                else          e.list_val.push_front(v);
            }
    ...
    // LPOP / RPOP — returns up to `count` elements
    std::vector<std::string> list_pop(const std::string& key,
        ...
        std::vector<std::string> result;
        {
            ...
            while (count-- > 0 && !lst.empty()) {
                if (dir >= 0) { result.push_back(lst.front()); lst.pop_front(); }
                else          { result.push_back(lst.back());  lst.pop_back();  }
            }
            if (lst.empty()) data_.erase(it);
        }
        if (!result.empty()) watch_reg.notify_write(key);
        return result;
    }
    ...
    long long llen(const std::string& key) {
        std::lock_guard<std::mutex> lk(mu);
        auto it = data_.find(key);
        if (it == data_.end() || it->second.type != StoreEntry::Type::List) return 0;
        return (long long)it->second.list_val.size();
    }

    // BLPOP: blocks until an element is available or timeout expires.
    // timeout_ms == 0 → block forever. Returns {key, value} or nullopt.
        ...
        while (true) {
            ...
            for (auto& k : keys_list) {
                ...
                if (it != data_.end() &&
                    ...
                    if (it->second.list_val.empty()) data_.erase(it);
            ...
            if (deadline) {
                ...
                if (status == std::cv_status::timeout) return std::nullopt;
            } else {
                cv.wait(lk);
            }
        }
    ...
    std::string xadd(const std::string& key, std::string id,
        ...
        std::string final_id;
        {
            ...
            if (ms < last_ms || (ms == last_ms && seq <= last_seq)) {
                if (ms == 0 && seq == 0) return "";
                if (!(ms == 0 && seq == 0 && last_ms == 0 && last_seq == 0)) return "";
            }

            final_id = std::to_string(ms) + "-" + std::to_string(seq);
            StreamEntry se; se.id = final_id; se.fields = fields;
    ...
    // Blocking XREAD: blocks until new entries appear after after_id, or timeout
    ...
    long long xlen(const std::string& key) {
        std::lock_guard<std::mutex> lk(mu);
        auto it = data_.find(key);
        if (it == data_.end() || it->second.type != StoreEntry::Type::Stream) return 0;
    ...
    long long zadd(const std::string& key,
        ...
        if (e.type != StoreEntry::Type::ZSet && e.type != StoreEntry::Type::String)
            return 0;
        if (e.type == StoreEntry::Type::String && !e.str_val.empty()) return 0;
        e.type = StoreEntry::Type::ZSet;
        long long added = 0;
        for (auto& [score, member] : members)
            if (e.zset_val.zadd(member, score)) ++added;
        return added;
    }
    ...
    long long zcard(const std::string& key) {
        std::lock_guard<std::mutex> lk(mu);
        auto it = data_.find(key);
        if (it == data_.end() || it->second.type != StoreEntry::Type::ZSet) return 0;
    ...
    long long zrem(const std::string& key,
        ...
        long long cnt = 0;
        for (auto& m : members)
            if (it->second.zset_val.zrem(m)) ++cnt;
        return cnt;
    }

    // ── Geo ops (stored inside ZSetData via geohash score) ───────────────────
    // GEOADD — stores lon/lat encoded as geohash52 in a zset
    long long geoadd(const std::string& key,
        ...
        if (e.type != StoreEntry::Type::ZSet && e.type != StoreEntry::Type::String)
            return -1;
        if (e.type == StoreEntry::Type::String && !e.str_val.empty()) return -1;
        e.type = StoreEntry::Type::ZSet;
        long long added = 0;
    ...
    // GEOPOS — decode lon/lat from geohash score
    ...
    // GEOSEARCH FROMMEMBER BYRADIUS — returns members within radius_m
    std::vector<std::string> geosearch_radius(const std::string& key,
        ...
        for (auto& [m, score] : it->second.zset_val.scores) {
            ...
            if (dist <= radius_m) result.push_back(m);
        }
        return result;
    }
...
private:
    ...
    static bool matches(const std::string& str, const std::string& pat) {
        ...
        while (s < str.size()) {
            ...
            else if (p < pat.size() && pat[p] == '*') { star_p = p++; star_s = s; }
            else if (star_p != std::string::npos) { p = star_p + 1; s = ++star_s; }
            else return false;
        }
    ...
    static int cmp_id(const std::string& a, const std::string& b) {
        ...
        if (a_ms != b_ms) return a_ms < b_ms ? -1 : 1;
        if (a_seq != b_seq) return a_seq < b_seq ? -1 : 1;
        return 0;
    }
